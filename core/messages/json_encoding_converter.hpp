...
class json_encoding_converter<std::map<key_t, value_t>> {
private:
	void exec_encode_wrapper(std::reference_wrapper<std::map<key_t, value_t>> obj,
							 const std::function<void(std::pair<key_t, value_t>&)>& op) {
		auto& m = obj.get();
		std::for_each(m.begin(), m.end(), op);
	}

public:
	void encode(std::reference_wrapper<std::map<key_t, value_t>> obj) {
		exec_encode_wrapper(obj, [&](std::pair<key_t, value_t>& val) {
			json_encoding_converter<std::pair<key_t, value_t>>().encode(std::ref(val));
		});
	}

	void decode(std::reference_wrapper<std::map<key_t, value_t>> obj) {
		exec_encode_wrapper(obj, [&](std::pair<key_t, value_t>& val) {
			json_encoding_converter<std::pair<key_t, value_t>>().decode(std::ref(val));
		});
	}
};