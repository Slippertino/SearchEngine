...
private:
	struct page_relevance_estimation_info : context {
		...
		SE_CONTEXT(
			page_relevance_estimation_info, -1,
			page_id,
			words_ids
		)
...
protected:
	void add_subscriptions(const service_ptr& service) const override {
		...
		service->router->subscribe<pages_selection_response>(service);
		service->router->subscribe<page_content_response>(service);
		service->router->subscribe<page_relevance_response>(service);
	}
